...
var GenTextures = function GenTextures(tileSize) {
    ...
    var betonImg = beton.brick(8, 8).normalize(0.6, 1).forBuf(betonNoise, function (a, b) {
        return a * b;
    }).getColor((0, _utils.randColor)([160, 160, 160]));

    var betonMask = new _buffer2.default(tileSize * 8);
    this.beton = betonMask.brickMask(8, 8).gaussian(step * 0.5 | 0).clamp(0.1, 0.3).normalize(0, 1).getColorLerp(betonImg, cementImg);
    ...
    var createSparkBrick = function createSparkBrick() {
        ...
        sparkBrickMask.normDist((0, _utils.rand)(0.75, 0.25)).normalize(0, 1).clamp(0.1, 0.3).normalize(0, 1);

        var sparkNoise = new _buffer2.default(tileSize * 0.5 | 0);
        sparkNoise.perlin(5, 0.5).diff([1, 0.5]).normalize(0.6, 1.4);

        var random = Math.random() * 0.3 + 0.7;

        var spark = new _buffer2.default(tileSize * 0.5 | 0);
        spark.forEach(function (a, i, j) {
            var x = (i / sparkBrickMask.size - 0.5) * 2;
            var y = (j / sparkBrickMask.size - 0.5) * 2;
            var factorX = 1 - 25 / 16 * x * x;
            var factorY = 1 - 25 / 16 * y * y;

            var dx = offsetX.getData(i, j);
            var dy = offsetY.getData(i, j);
            var tx = (0, _utils.clamp)(i + dx * step * 0.25, 0, sparkBrickMask.size) | 0;
            var ty = (0, _utils.clamp)(j + dy * step * 0.25, 0, sparkBrickMask.size) | 0;
        ...
        return new _buffer2.default(tileSize * 0.5 | 0).forBuf(spark, function (a, b) {
            ...
            return a * b;
        }).getColor((0, _utils.randColor)([200, 80, 60]), spark);
    };

    this.sparksBrick = [];
    for (var _i12 = 0; _i12 < 10; _i12++) {
        this.sparksBrick[_i12] = [];
        this.sparksBrick[_i12].push(createSparkBrick());

        var COUNT_ANGLES = 32;
        for (var _angle = 1; _angle < COUNT_ANGLES; _angle++) {
            this.sparksBrick[_i12].push(rotateImage(this.sparksBrick[_i12][0], _angle / COUNT_ANGLES * 4));
        }
    }
            ...
            var decrementLevel = function decrementLevel(x, y) {
                ...
                if (tile) {
                    ...
                    if (needHalf) {
                        ...
                        _this2.particles.emit(tile.x - 0.5, tile.y - 0.5, _global.PART.BRICK);
                    } else if (needEmpty) {
    ...
    _createClass(Particle, [{
            ...
            if (this.alive) {
                switch (this.type) {
                    ...
                    case _global.PART.BRICK:
                        {
                            var _id = this.random * level.textures.sparksBrick.length | 0;
                            var _ind2 = (this.invAngle * this.deltatime / 30 + level.textures.sparksBrick[_id].length * this.startAngle | 0) % level.textures.sparksBrick[_id].length | 0;
                            this.entity.vel = this.maxvel * (1 - this.deltatime / this.lifetime);
                            level.drawEntity(this.entity, level.textures.sparksBrick[_id][_ind2]);
                            break;
                        }
                }
            }
        ...
        value: function update(level, delta) {
            ...
            if (this.deltatime >= this.lifetime) {
                this.alive = false;
                if (this.type === _global.PART.BRICK) {
                    var id = this.random * level.textures.sparksBrick.length | 0;
                    var ind = (this.invAngle * this.deltatime / 30 + level.textures.sparksBrick[id].length * this.startAngle | 0) % level.textures.sparksBrick[id].length | 0;
...
var ParticleManager = function (_EntityManager) {
    ...
    _createClass(ParticleManager, [{
        ...
        value: function emit(cx, cy, type) {
            var count = 1;
            if (type === _global.PART.SPARK) count = 5;else if (type === _global.PART.BRICK) count = 10;

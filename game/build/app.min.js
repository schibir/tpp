...
var GenTextures = function GenTextures(tileSize) {
    ...
    this.brick = brickMask.brickMask(8, 16).gaussian(step * 0.5 | 0).clamp(0.1, 0.3).normalize(0, 1).getColorLerp(brickImg, cementImg);
    ...
    var betonImg = beton.brick(4, 4).normalize(0.6, 1).forBuf(betonNoise, function (a, b) {
        return a * b;
    }).getColor((0, _utils.randColor)([160, 160, 160]));

    var betonMask = new _buffer2.default(tileSize * 8);
    this.beton = betonMask.brickMask(4, 4).gaussian(step * 0.5 | 0).clamp(0.1, 0.3).normalize(0, 1).getColorLerp(betonImg, cementImg);
...
var Tank = exports.Tank = function (_Entity) {
    ...
    function Tank(cx, cy, type) {
        ...
        _this.timeRespawn = Date.now();
        _this.guanoRespawn = new _entity2.default(cx + 4, cy);
        return _this;
    }
    ...
    _createClass(Tank, [{
        ...
        value: function clear(level) {
            ...
            level.clearEntity(this.guanoRespawn);
        }
    }, {
        key: "draw",
        value: function draw(level) {
            ...
            var ind = (Date.now() - this.timeRespawn) / 50 % level.textures.respawn.length | 0;
            level.drawEntity(this.guanoRespawn, level.textures.respawn[ind]);
        }
    }, {
        key: "update",
        value: function update(level, delta) {
            ...
            this.guanoRespawn.cx = this.cx + 4;
            this.guanoRespawn.cy = this.cy;
        }
    }]);

...
var GenTextures = function GenTextures(tileSize) {
    ...
    var createCorpus = function createCorpus(size) {
        ...
        corpusMask.forEach(function (a, i, j) {
            var x = (i / corpusMask.size - 0.5) * 2;
            var y = (j / corpusMask.size - 0.5) * 2;
            var factorX = Math.abs(x) < 0.6 ? 1 : 0;
            var factorY = Math.abs(y) < size ? 1 : 0;
            return factorX * factorY;
        }).gaussian(step).clamp(0.5, 0.6).normalize(0, 1);
    ...
    var createTurret = function createTurret(size) {
        var turretMask = new _buffer2.default(tileSize * 2);
        turretMask.forEach(function (a, i, j) {
            var x = (i / turretMask.size - 0.5) * 2;
            var y = (j / turretMask.size - 0.5) * 2;
            var factorX = Math.abs(x) < size ? 1 : 0;
            var factorY = Math.abs(y) < size ? 1 : 0;
            return factorX * factorY;
        }).gaussian(step).clamp(0.5, 0.6).normalize(0, 1);
        ...
        return turret.forEach(function (a, i, j) {
            var x = (i / turretMask.size - 0.5) * 2;
            var y = (j / turretMask.size - 0.5) * 2;
            var factorX = Math.abs(x) < size ? 1 : 0;
            var factorY = Math.abs(y) < size ? 1 : 0;
            return factorX * factorY;
        }).gaussian(step).normalize(0, 1).getColor([67, 114, 61], turretMask);
    };
...
var Level = exports.Level = function Level(levelName, canvas) {
    ...
    loadLevel(function () {
        ...
        _this.context.drawImage(_this.textures.trackVel, 550, 600);
        _this.context.drawImage(_this.textures.corpusEasy, 550, 600);

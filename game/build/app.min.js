...
var GenTextures = function GenTextures(tileSize) {
    ...
    var createCorpus = function createCorpus(size, color) {
        ...
        return corpus.brick(10, 10).normalize(0.5, 1).forEach(function (a, i, j) {
            var y = (j / corpus.size - 0.5) * 2;
            if (Math.abs(y) < size - 0.25) return a;
            var k = (0, _utils.clamp)((size - Math.abs(y)) / 0.25, 0, 1);
            return a * Math.sqrt(k);
        }).normalize(0.5, 1).getColor(color, corpusMask);
    };
    var createTurret = function createTurret(size, barrelWidth, barrelLength, color) {
        ...
        var barrelImg = smoothedSquare(barrel, barrelWidth, barrelLength).normalize(-1, 1).getColor([200, 200, 200], barrelMask);
        ...
        var turretImg = smoothedSquare(turret, size, size).normalize(0, 1).getColor(color, turretMask);
    ...
    var easyColor = [67, 114, 61];
    var BMPColor = [172, 172, 172];
    var longColor = [226, 210, 104];
    var strongColor = [74, 186, 169];
    var panzerColor = [211, 229, 224];
    var player1Color = [168, 128, 69];

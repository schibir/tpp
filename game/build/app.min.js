...
var SimpleBuffer = function () {
    ...
    _createClass(SimpleBuffer, [{
        ...
        value: function getColor2() {
            ...
            var imageData = context.createImageData(this.size, this.size);
            for (var i = 0; i < this.size * this.size; i++) {
                imageData.data[4 * i + 0] = (0, _utils.clamp)((0, _utils.lerp)(col0[0], col1[0], this.data[i]), 0, 255) | 0;
                imageData.data[4 * i + 1] = (0, _utils.clamp)((0, _utils.lerp)(col0[1], col1[1], this.data[i]), 0, 255) | 0;
                imageData.data[4 * i + 2] = (0, _utils.clamp)((0, _utils.lerp)(col0[2], col1[2], this.data[i]), 0, 255) | 0;
                imageData.data[4 * i + 3] = 255;
            }
            context.putImageData(imageData, 0, 0);
            return canvas;
        ...
        value: function getColorLerp(img0, img1) {
            ...
            var imageData = context.createImageData(this.size, this.size);
            for (var i = 0; i < this.size * this.size; i++) {
                imageData.data[4 * i + 0] = (0, _utils.lerp)(data0[4 * i + 0], data1[4 * i + 0], this.data[i]) | 0;
                imageData.data[4 * i + 1] = (0, _utils.lerp)(data0[4 * i + 1], data1[4 * i + 1], this.data[i]) | 0;
                imageData.data[4 * i + 2] = (0, _utils.lerp)(data0[4 * i + 2], data1[4 * i + 2], this.data[i]) | 0;
                imageData.data[4 * i + 3] = 255;
            }
            context.putImageData(imageData, 0, 0);
            return canvas;
...
function main() {
    ...
    var noise = new _buffer2.default(256);
    noise.perlin(5, 0.5).normalize(0, 1);

    var ground = new _buffer2.default(256);
    var groundImg = ground.perlin(5, 0.9).normalize(0.75, 1).getColor((0, _utils.randColor)([224, 207, 159]));

    var cement = new _buffer2.default(256);
    var cementImg = cement.perlin(5, 0.5).diff([1, 0.5]).diff([-0.5, 1]).normalize(0, 1).getColor((0, _utils.randColor)([100, 100, 100]));

    var brick = new _buffer2.default(256);
    var brickImg = brick.brick(4, 8).forBuf(noise, function (a, b) {
        return a * b;
    }).normalize(0.7, 1).getColor((0, _utils.randColor)([160, 54, 35]));

    var brickMask = new _buffer2.default(256);
    brickMask.brickMask(4, 8).gaussian(3).clamp(0.1, 0.3).normalize(0, 1);

    var brickCementImg = brickMask.getColorLerp(brickImg, cementImg);

    var betonNoise = new _buffer2.default(256);
    betonNoise.perlin(5, 0.5).forEach(function (a) {
        return a * a;
    }).diffFree().normalize(0.6, 1);

    var beton = new _buffer2.default(256);
    var betonImg = beton.brick(4, 4).normalize(0.7, 1).forBuf(betonNoise, function (a, b) {
        return a * b;
    }).getColor((0, _utils.randColor)([160, 160, 160]));

    var betonMask = new _buffer2.default(256);
    betonMask.brickMask(4, 4).gaussian(3).clamp(0.1, 0.3).normalize(0, 1);

    var betonCementImg = betonMask.getColorLerp(betonImg, cementImg);

    var lava = new _buffer2.default(256);
    var lavaImg = lava.perlin(2, 0.5).normalize(0, 30).forEach(Math.cos).normalize(0.5, 1).getColor2((0, _utils.randColor)([255, 0, 0]), (0, _utils.randColor)([255, 255, 0]));

    // grass
    var grass = new _buffer2.default(128);
    grass.perlin(5, 0.9).normalize(0.7, 1);

    var grassImg = grass.getColor((0, _utils.randColor)([49, 107, 54]));

...
function main() {
    ...
    var groundImg = ground.perlin(2, 0.5).diff([-0.5, 1]).diff([1, 0.5]).normalize(0.75, 1).getColor((0, _utils.randColor)([224, 207, 159], 20));

    var cement = new _buffer2.default(256);
    var cementImg = cement.perlin(5, 0.5).diff([1, 0.5]).diff([-0.5, 1]).normalize(0, 1).getColor((0, _utils.randColor)([100, 100, 100], 10));

    var brick = new _buffer2.default(256);
    var brickImg = brick.brick(5, 10).forBuf(noise, function (a, b) {
        return a * b;
    }).normalize(0.7, 1).getColor((0, _utils.randColor)([160, 54, 35], 10));

    var brickMask = new _buffer2.default(256);
    brickMask.brickMask(5, 10).gaussian(3).clamp(0.1, 0.3).normalize(0, 1);
    ...
    betonNoise.perlin(5, 0.5).forEach(function (a) {
        return a * a;
    }).diffFree().normalize(0.5, 1);

    var beton = new _buffer2.default(256);
    var betonImg = beton.brick(5, 5).normalize(0.7, 1).forBuf(betonNoise, function (a, b) {
        return a * b;
    }).getColor((0, _utils.randColor)([160, 160, 160], 10));

    var betonMask = new _buffer2.default(256);
    betonMask.brickMask(5, 5).gaussian(3).clamp(0.1, 0.3).normalize(0, 1);

...
var GenTextures = function GenTextures(tileSize) {
    ...
    for (var i = 0; i < this.lavaMask.length; i++) {
        ...
        this.lavaMask[i] = lavaMask.normSquare(0.5, 0.5).normalize(0, 1).forBuf(lavaNoise, function (a, b) {
            return a + 0.25 * b;
        }).clamp(0.4, 0.6).normalize(0, 1).getColor([255, 255, 255], lavaMask);

        var lavaLightMask = new _buffer2.default(tileSize * 2);
        this.lavaLightMask[i] = lavaLightMask.normSquare(0.5, 0.5).normalize(0, 1.5).forBuf(lavaNoise, function (a, b) {
            return a + 0.25 * b;
        }).clamp(0.25, 0.75).gaussian(7).normalize(0, 1).getColor([255, 255, 255], lavaLightMask);
    }
    ...
    this.grass = grassMask.perlin(20, 0.9).normalize(0, 1).clamp(0.2, 0.5).normalize(0, 1).getColor((0, _utils.randColor)([49, 107, 54], grassMask));
    ...
    var renderLava = function renderLava() {
        ...
        _this.map.forEach(function (tile, index) {
            if (tile & (WATER | BRIDGE)) {
                ...
                var posX = (x - 1) * tileSize + tileSize / 2 | 0;
                var posY = (y - 1) * tileSize + tileSize / 2 | 0;
        ...
        var lava = _this.textures.lava;

        for (var y = 0; y < _this.layerLava.canvas.height; y += lava.height) {
            for (var x = 0; x < _this.layerLava.canvas.width; x += lava.width) {
                _this.layerLava.context.drawImage(lava, x, y);
            }
        }

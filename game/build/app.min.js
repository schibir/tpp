...
var GenTextures = function GenTextures(tileSize) {
    ...
    function createSpark() {
        ...
        for (var _i7 = 0; _i7 < COUNT_PART; _i7++) {
            var sx = (0, _utils.rand)(0, SIZE / COUNT_FRAMES / 2);
            var sy = (0, _utils.rand)(0, SIZE / COUNT_FRAMES / 2);
            var px = SIZE / 2;
            var py = SIZE / 2;
            pos[_i7] = [px, py];
            vel[_i7] = [sx, sy];
            len[_i7] = (Math.random() + 0.5) * LENGTH;
        }
        ...
        for (var _i8 = 0; _i8 < COUNT_FRAMES; _i8++) {
            ...
            for (var j = 0; j < COUNT_PART; j++) {
                var _x2 = pos[j][0] | 0;
                var _y = pos[j][1] | 0;
                buf.bresenham(_x2, _y, _x2 - vel[j][0] * len[j] | 0, _y - vel[j][1] * len[j] | 0, 1);
                pos[j][0] += vel[j][0];
                pos[j][1] += vel[j][1];
            }
            buf.gaussian(step).clamp(0, 0.2).normalize(0, 2 - 2 * _i8 / COUNT_FRAMES);
            ret.push(buf);
        }
        return ret;
    }
